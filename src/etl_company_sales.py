...
====================================
This script performs ETL operations on company sales data including:
1. Data ingestion from CSV
2. Data cleaning (trim, type conversion, date standardization)
3. Data quality checks (4 validation rules)
...
def clean_company_sales(df, spark):
    """
    ...
    df_clean = df_clean.fillna({"units_sold": 0, "revenue": 0.0})
...
def apply_dq_checks(df, spark):
    """
    Apply data quality checks for company sales:
    ...
    good_df = df_dq.filter((F.col("dq_failure_reason") == "") | (F.col("dq_failure_reason").isNull())) \
                   .drop("dq_item_id_null", "dq_units_sold_negative", "dq_revenue_negative",
                         "dq_sale_date_invalid", "dq_failure_reason")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='ETL pipeline for company sales data')
    parser.add_argument('--config', required=True, help='Path to YAML configuration file')
    args = parser.parse_args()

    logger.info("=" * 80)
    logger.info("Starting Company Sales ETL Pipeline")
    logger.info("=" * 80)
    ...
    input_path = company_config['input_path']
    hudi_output_path = company_config['hudi_output_path']
    quarantine_path = company_config['quarantine_path']

    # Create Spark session
    spark = get_spark_session("EcommerceRecommendation_CompanySales_ETL")
    ...
    try:
        ...
        # Clean data
        df_clean = clean_company_sales(df_raw, spark)

        # Apply DQ checks
        df_good, df_bad = apply_dq_checks(df_clean, spark)

        # Write bad records to quarantine
        write_to_quarantine(df_bad, quarantine_path, "company_sales")

        # Write good records to Hudi
        write_to_hudi(df_good, hudi_output_path, "company_sales")

        logger.info("=" * 80)
        logger.info("Company Sales ETL Pipeline Completed Successfully")
        logger.info("=" * 80)
